...
import {
  normalizeRecipientsFromLegacyTarget,
  validateNotificationRecipients,
} from 'src/app/utils/validate-notification-recipients.util';
...
export class NotificationRequestsService extends BaseService<NotificationRequestData> {
  ...
  getStats(query: NotificationRequestStatsQueryDto): Promise<NotificationRequestStats> {
    return this.notificationRequestRepository.getStats({
      clientSystemId: query.clientSystemId,
      requestedFrom: query.requestedFrom ? new Date(query.requestedFrom) : undefined,
  ...
  async getDetail(id: string) {
    ...
    const [recipients, attachments, deliveries, audits, attempts] =
      await Promise.all([
        this.notificationRecipientRepository.findAllByNotificationRequestId(id),
    ...
    return { request, recipients, attachments, deliveries, audits, attempts };
  }
  ...
  async submit(clientSystemId: string, dto: CreateNotificationRequestDto) {
    ...
    const recipients = normalizeRecipientsFromLegacyTarget({
      target: dto.target,
      recipients: dto.recipients,
    });
    const recipientValidation = validateNotificationRecipients(recipients);
    ...
    const savedRecipients = await this.notificationRecipientRepository.createMany(
  ...
  async processRequest(notificationRequestId: string, job?: Job) {
    ...
    if (request.status !== NotificationRequestStatus.PROCESSING) {
      await this.transitionStatus(request, NotificationRequestStatus.PROCESSING, {
    ...
    const [recipients, attachments, deliveries] = await Promise.all([
      this.notificationRecipientRepository.findAllByNotificationRequestId(
        notificationRequestId,
      ),
    ...
    const deliveryByRecipient = new Map(
      deliveries.map((delivery) => [delivery.notificationRecipientId, delivery]),
    );
    ...
    for (const recipient of recipients) {
      ...
      try {
        ...
        await this.notificationRecipientRepository.update(recipient.id, {
    ...
    if (
      refreshed &&
      [NotificationRequestStatus.FAILED, NotificationRequestStatus.PARTIAL].includes(
        refreshed.status,
      ) &&
      job &&
  ...
  async retryFailedDeliveries(id: string) {
    ...
    if (!failedDeliveries.length) {
      throw new BadRequestException('No hay deliveries fallidos para reintentar');
    }
  ...
  private async removeQueueJob(notificationRequestId: string) {
    const job = await this.notificationRequestQueue.getJob(notificationRequestId);
  ...
  private buildStoredPayload(
    ...
    return {
      channel: channelCode,
      ...(dto.target?.trim() ? { target: dto.target.trim() } : {}),
      ...dto.payload,
    };
  }
